...
class QuanticsCalculation:
    ...
    def from_dict(cls, data: Dict):
        ...
        calc.working_directory = Path(data['working_directory']) if data['working_directory'] else None
...
class LocalQuanticsRunner:
    ...
    def _load_calculations(self) -> Dict[str, QuanticsCalculation]:
        ...
        try:
            with open(self.config_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            return {name: QuanticsCalculation.from_dict(calc_data) 
                   for name, calc_data in data.items()}
    ...
    def create_calculation(self, name: str, inp_file: Path, op_file: Path, 
                          workflow_type: str = "MCTDH", 
                          db_folder: Optional[Path] = None) -> QuanticsCalculation:
    ...
    def run_calculation(self, calc: QuanticsCalculation, quantics_executable: str = "quantics") -> bool:
    ...
    def run_analysis_with_params(self, calc: QuanticsCalculation, analysis_tools: List[str], 
    ...
    def print_status(self):
        ...
        for calc in self.calculations.values():
            start_time = calc.start_time.strftime("%Y-%m-%d %H:%M:%S") if calc.start_time else "Not started"
...
def main():
    ...
    create_parser.add_argument("--workflow", choices=["MCTDH", "vMCG", "DD-vMCG"], 
                              default="MCTDH", help="Workflow type")
    ...
    if args.command == "create":
        try:
            calc = runner.create_calculation(
                args.name, args.inp_file, args.op_file, 
                args.workflow, args.db_folder
            )
