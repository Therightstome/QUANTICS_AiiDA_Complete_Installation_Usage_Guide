...
class QuanticsAiidaCalculation(CalcJob):
    ...
    def define(cls, spec):
        ...
        spec.input('metadata.options.max_wallclock_seconds', 
                  valid_type=int, 
                  default=3600)
        
        # QUANTICS specific inputs
        spec.input('inp_file', valid_type=orm.SinglefileData, 
                  help='QUANTICS input file (.inp)')
        spec.input('op_file', valid_type=orm.SinglefileData, 
                  help='QUANTICS operator file (.op)')
        spec.input('db_folder', valid_type=orm.FolderData, 
                  required=False, 
                  help='Database folder for DD-vMCG calculations')
        spec.input('workflow_type', valid_type=orm.Str, 
                  default=lambda: orm.Str('MCTDH'),
                  help='Workflow type: MCTDH, vMCG, or DD-vMCG')
        spec.input('analysis_tools', valid_type=orm.List, 
                  required=False,
                  help='List of analysis tools to run')
        spec.input('analysis_params', valid_type=orm.Dict, 
                  required=False,
                  help='Parameters for analysis tools')
        
        # Define outputs
        spec.output('output_folder', valid_type=orm.FolderData, 
                   help='Complete output folder')
        spec.output('results', valid_type=orm.Dict, 
                   help='Parsed calculation results')
        spec.output('analysis_results', valid_type=orm.Dict, 
                   required=False,
                   help='Analysis tool results')
        
        # Define exit codes
        spec.exit_code(100, 'ERROR_MISSING_INPUT_FILES', 
                      message='Required input files not found')
        spec.exit_code(200, 'ERROR_QUANTICS_FAILED', 
                      message='QUANTICS calculation failed')
        spec.exit_code(300, 'ERROR_ANALYSIS_FAILED', 
                      message='Analysis tools failed')
...
"""
        ...
        if 'analysis_tools' in self.inputs:
            ...
            analysis_params = self.inputs.analysis_params.get_dict() if 'analysis_params' in self.inputs else {}
            ...
            if self.retrieved.list_object_names('output'):
                ...
                for filename in self.retrieved.list_object_names('output'):
                    output_folder_node.put_object_from_filelike(
                        self.retrieved.open(f'output/{filename}', 'rb'),
                        filename
                    )
...
class QuanticsAiidaIntegration:
    ...
    def _setup_code(self, code_label: str = None):
        ...
        try:
            ...
            if available_codes:
                ...
                self.report("verdi code setup --label quantics --computer localhost --remote-abs-path /path/to/quantics")
    ...
    def submit_calculation(self, 
                          ...
                          analysis_params: Optional[Dict] = None) -> orm.ProcessNode:
        """
    ...
    def get_calculation_status(self, process_pk: int) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                'state': process.process_state.value if hasattr(process, 'process_state') else 'unknown',
...
def setup_aiida_for_quantics():
    ...
    print("   verdi code setup --label quantics --computer localhost --remote-abs-path $(which quantics)")
